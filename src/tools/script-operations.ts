...
export class ScriptOperationsTools {
  ...
  async createScript(input: unknown): Promise<CreateScriptResult> {
    ...
    try {
      ...
      if (attachToScene) {
        await attachScriptToScene(
          validatedProjectPath,
          attachToScene.scenePath,
          attachToScene.nodePath,
          relativeScriptPath
        );
      }
  ...
  async modifyScript(input: unknown): Promise<ModifyScriptResult> {
  ...
  async validateScript(input: unknown): Promise<ValidateScriptResult> {
...
function buildTemplateContent(template: z.infer<typeof ScriptTemplateSchema>, scriptPath: string): string {
...
function buildCSharpTemplate(className: string): string {
...
function templateParentClass(template: z.infer<typeof ScriptTemplateSchema>): string {
  switch (template) {
    case 'character_body_2d':
...
function toClassName(scriptPath: string): string {
...
function applyScriptChanges(content: string, changes: ScriptChange[]): string {
...
async function validateScriptFile(scriptPath: string, editorPath?: string): Promise<ValidateScriptResult> {
...
function basicValidateGDScript(content: string): { errors: string[]; warnings: string[] } {
  const errors: string[] = [];
...
function countBalance(content: string, openChar: string, closeChar: string): number {
  let balance = 0;
...
async function attachScriptToScene(
  projectPath: string,
  scenePath: string,
  nodePath: string,
  scriptPath: string
...
function ensureScriptExtResource(content: string, scriptResPath: string): { updatedContent: string; resourceId: string } {
  const lines = content.split('\n');
...
function applyScriptProperty(content: string, nodePath: string, resourceId: string): string {
  const lines = content.split('\n');
...
function findNodeSection(lines: string[], nodePath: string): number {
  if (nodePath === '.') {
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i]!.trim();
