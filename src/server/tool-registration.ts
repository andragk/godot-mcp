...
} from '../tools/script-operations.js';
 ...
 */
export function registerConnectivityTools(
  ...
  registry.register({
    ...
    handler: async (_args, correlationId) => {
      ...
      const result = await godotClient.sendRequest<{ status: string }>('ping');
      return result;
    },
  });
...
export function registerEditorTools(
  registry: ToolRegistry,
  editorTools: EditorControlTools
): void {
  registry.register({
    metadata: {
 ...
 */
export function registerNodeTools(registry: ToolRegistry): void {
  registry.register({
    metadata: {
      name: 'search_nodes',
      version: '1.0.0',
 ...
 */
export function registerScriptTools(registry: ToolRegistry): void {
