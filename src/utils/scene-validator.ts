...
export class SceneValidator {
  ...
  private validateStructure(
    ...
    const lines = content.split('\n');
    const nodes: Array<{ name: string; parent: string | null; line: number }> = [];
    ...
    nodes.forEach(node => {
      if (node.parent && node.parent !== '.') {
        ...
        const nodePathMap = new Map<string, boolean>();
        nodes.forEach(n => {
          if (n.line < node.line) {
  ...
  private validateReferences(
    ...
    const extResourceIds = new Set<string>();
    const subResourceIds = new Set<string>();
    const usedExtResources = new Set<string>();
    const usedSubResources = new Set<string>();
