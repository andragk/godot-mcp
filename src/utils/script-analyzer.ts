 ...
 */
export function analyzeScript(content: string): ScriptMetadata {
 ...
 */
export function analyzeCSharpScript(content: string): ScriptMetadata {
  const lines = content.split('\n');
  const metadata: ScriptMetadata = {
    functions: [],
    signals: [],
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const methodMatch = line.match(
      /^(?:public|private|protected|internal|static|virtual|override|async|sealed|new|partial|extern|unsafe|readonly|\s)+\s*([\w<>\[\]]+)\s+(\w+)\s*\(([^)]*)\)/
    );
    if (methodMatch) {
 ...
 */
function parseFunctionSignature(line: string, lineNumber: number): FunctionSignature | null {
 ...
 */
export function extractFunctionAtLine(content: string, targetLine: number): string | null {
  const lines = content.split('\n');
...
export function calculateComplexity(content: string, metadata: ScriptMetadata): ComplexityMetrics {
  const lines = content.split('\n');
 ...
 */
export function findReferences(content: string, identifier: string): number[] {
  const lines = content.split('\n');
 ...
 */
export function extractTypeDependencies(content: string): string[] {
