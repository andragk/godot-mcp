 ...
 */
function getDangerousPatterns(allowAbsolute: boolean): RegExp[] {
...
export async function validatePath(
  ...
  if (requiredPermissions.length > 0) {
    ...
    try {
      ...
      throw new ValidationError(
        `Insufficient permissions for path: ${path} (required: ${requiredPermissions.join(', ')})`
      );
    }
  }
...
export function validatePathSync(
  path: string,
  options: Omit<PathValidationOptions, 'mustExist' | 'requiredPermissions'> = {}
): string {
  const {
    baseDir,
    allowAbsolute = false,
