 ...
 */
export function validateGodotArguments(args) {
    const validated = [];
    let i = 0;
    while (i < args.length) {
        const arg = args[i];
...
function validateArgumentValue(argName, value) {
    ...
    for (const pattern of DANGEROUS_VALUE_PATTERNS) {
        if (pattern.test(value)) {
            throw new ValidationError(`Argument ${argName} contains dangerous pattern in value: ${value}`);
        }
    }
    ...
    switch (argName) {
        case '--resolution':
            if (!/^\d+x\d+$/.test(value)) {
                throw new ValidationError(`Invalid resolution format: ${value} (expected: WIDTHxHEIGHT, e.g., 1920x1080)`);
            }
            break;
        case '--position':
            if (!/^\d+,\d+$/.test(value)) {
                throw new ValidationError(`Invalid position format: ${value} (expected: X,Y, e.g., 100,100)`);
            }
            break;
        case '--screen':
        case '--fixed-fps':
        case '--frame-delay':
            if (!/^\d+$/.test(value)) {
                throw new ValidationError(`Argument ${argName} requires a numeric value, got: ${value}`);
            }
            break;
        case '--time-scale':
            if (!/^\d+\.?\d*$/.test(value)) {
 ...
 */
export function prepareGodotCommand(executable, args) {
    return {
        command: executable,
 ...
 */
export function isArgumentAllowed(arg) {
    const argName = arg.includes('=') ? arg.split('=')[0] : arg;
