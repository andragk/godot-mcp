...
export class SceneOperationsTools {
     ...
     */
    applyRemoveNode(sceneContent, operation) {
     ...
     */
    applyAddNode(sceneContent, operation) {
        ...
        const existingIndex = this.findNodeSection(lines, parentPath === '.' ? node.name : `${parentPath}/${node.name}`);
        if (existingIndex !== -1) {
            throw new ValidationError(`Node '${node.name}' already exists under parent '${parentPath}'`);
        }
     ...
     */
    applyRenameNode(sceneContent, operation) {
        ...
        if (duplicateIndex !== -1 && duplicateIndex !== nodeIndex) {
            throw new ValidationError(`A node named '${newName}' already exists under parent '${parentPath}'`);
        }
        // Replace the name in the node header
        const line = lines[nodeIndex];
        const updatedLine = line.replace(/name="[^"]+"/, `name="${newName}"`);
     ...
     */
    applyReparentNode(sceneContent, operation) {
        ...
        if (newParentPath === nodePath || newParentPath.startsWith(`${nodePath}/`)) {
            throw new ValidationError(`Cannot reparent node to itself or its descendant`);
        }
        // Get node name
        const pathParts = nodePath.split('/');
        ...
        const duplicateIndex = this.findNodeSection(lines, newPath);
        if (duplicateIndex !== -1 && duplicateIndex !== nodeIndex) {
            throw new ValidationError(`A node named '${nodeName}' already exists under parent '${newParentPath}'`);
        }
        ...
        if (newParentPath === '.') {
            ...
            if (line.includes('parent=')) {
                ...
                lines[nodeIndex] = line.replace(/]$/, ` parent="."]`);
            }
        }
        else {
            ...
            if (line.includes('parent=')) {
                ...
                lines[nodeIndex] = line.replace(/]$/, ` parent="${newParentPath}"]`);
            }
        }
