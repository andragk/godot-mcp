...
import { parseSceneFile, buildNodeHierarchy } from '../utils/scene-parser.js';
 ...
 */
function searchNodesRecursive(node, query, mode, operator, scenePath, results, sceneNodes, parentPath = '') {
    ...
    if (node.children && node.children.length > 0) {
        for (const child of node.children) {
            searchNodesRecursive(child, query, mode, operator, scenePath, results, sceneNodes, nodePath);
        }
    }
}
/**
 ...
 */
function nodeMatchesQuery(node, sceneNode, query, mode, operator) {
    const criteria = [];
    // Name matching
    if (query.name) {
    ...
    // Type matching
    if (query.type) {
        const typeMatch = node.type === query.type ||
 ...
 */
function matchProperties(nodeProperties, queryProperties) {
 ...
 */
function calculateMatchScore(node, _sceneNode, query, mode) {
...
export async function getNodeProperties(projectPath, scenePath, nodePath) {
    ...
    const children = hierarchyNode.children.map(child => nodePath ? `${nodePath}/${child.name}` : child.name);
 ...
 */
function findNodeInHierarchy(root, targetPath) {
 ...
 */
function findNodeInChildren(children, parts) {
    if (parts.length === 0) {
        return null;
    }
...
function categorizeProperty(name) {
    if (name.includes('position') || name.includes('location'))
        return 'Transform';
    if (name.includes('rotation') || name.includes('scale'))
        return 'Transform';
    if (name.includes('color') || name.includes('modulate'))
        return 'Visual';
    if (name.includes('visible') || name.includes('opacity'))
        return 'Visual';
    if (name.includes('size') || name.includes('rect'))
        return 'Layout';
